...
class _TableCalendarBaseState extends State<TableCalendarBase>
  ...
  void didUpdateWidget(TableCalendarBase oldWidget) {
    ...
    if (widget.dowHeight != oldWidget.dowHeight) {
      final rowCount = _getRowCount(_focusedDay);
  ...
  void _updatePage({bool shouldAnimate = false}) {
    ...
    final endIndex = _calculateFocusedPage(
        widget.firstDay, widget.lastDay);
  ...
  double _getPageHeight(int rowCount) {
    final dowHeight = widget.dowHeight;
    return dowHeight + rowCount * 35;
  }
  ...
  int _getDaysBefore(DateTime firstDay) {
    return (firstDay.weekday + 7 - getWeekdayNumber(StartingDayOfWeek.monday)) % 7;
  }
